    ...
    public class ScottNetMappingProfile : Profile
    {
        public ScottNetMappingProfile()
        {
            ...
            CreateMap<ConsoleReadingViewModel, WeatherReadingViewModel>()
                .ForMember(se => se.Barometer, vm => vm.MapFrom(vmm => vmm.Barometer / 1000))
                .ForMember(se => se.IndoorTemp, vm => vm.MapFrom(vmm => vmm.IndoorTemp / 10))
                .ForMember(se => se.OutdoorTemp, vm => vm.MapFrom(vmm => vmm.OutdoorTemp / 10))
                .ForMember(se => se.WindSpeed10M, vm => vm.MapFrom(vmm => vmm.WindSpeed10M / 10))
                .ForMember(se => se.WindSpeed2M, vm => vm.MapFrom(vmm => vmm.WindSpeed2M / 10))
                .ForMember(se => se.RainRate, vm => vm.MapFrom(vmm => vmm.RainRate / 100))
                .ForMember(se => se.RainStorm, vm => vm.MapFrom(vmm => vmm.RainStorm / 100))
                .ForMember(se => se.RainDay, vm => vm.MapFrom(vmm => vmm.RainDay / 100))
                .ForMember(se => se.RainMonth, vm => vm.MapFrom(vmm => vmm.RainMonth / 100))
                .ForMember(se => se.RainYear, vm => vm.MapFrom(vmm => vmm.RainYear / 100))
                .ForMember(se => se.Rain24H, vm => vm.MapFrom(vmm => vmm.Rain24Hour / 100))
                .ForMember(se => se.Rain1H, vm => vm.MapFrom(vmm => vmm.Rain1Hour / 100))
